...
class Semantics
{
 private:
	...
	// semantic containers)
	static constexpr std::size_t N_HPP = 1 + (N - 1) / 2;
 ...
 public:
	...
	iterator begin() noexcept { return empty() ? nullptr : data_.get() + N_HPP; }

	const_iterator begin() const noexcept
	{
		return empty() ? nullptr : data_.get() + N_HPP;
	}
	...
	{
		return empty() ? nullptr : data_.get() + N_HPP + offset(index);
	}
	...
	{
		return empty() ? nullptr : data_.get() + N_HPP + offset(index) + size(index);
	}
	...
	[[nodiscard]] IteratorWrapper<iterator> iter(index_t const index)
	{
		return IteratorWrapper<iterator>(begin(index), end(index));
	}

	[[nodiscard]] IteratorWrapper<const_iterator> iter(index_t const index) const
	{
		return IteratorWrapper<const_iterator>(begin(index), end(index));
	}

	[[nodiscard]] IteratorWrapper<const_iterator> citer(index_t const index) const
	{
		return IteratorWrapper<const_iterator>(cbegin(index), cend(index));
	}

	[[nodiscard]] IteratorWrapper<iterator> riter(index_t const index)
	{
		return IteratorWrapper<iterator>(rbegin(index), rend(index));
	}

	[[nodiscard]] IteratorWrapper<const_iterator> riter(index_t const index) const
	{
		return IteratorWrapper<const_iterator>(rbegin(index), rend(index));
	}

	[[nodiscard]] IteratorWrapper<const_iterator> criter(index_t const index) const
	{
		return IteratorWrapper<const_iterator>(crbegin(index), crend(index));
	}
	...
	[[nodiscard]] size_type allocSize() const { return empty() ? 0 : size() + N_HPP; }
	...
	[[nodiscard]] Semantic const *data() const { return data_.get() + N_HPP; }
 ...
 protected:
	...
	[[nodiscard]] Semantic *data() { return data_.get() + N_HPP; }
	...
	[[nodiscard]] std::array<size_type, N> sizes() const
	{
		...
		if constexpr (0 == N % 2) {
			std::copy(data_.get(), data_.get() + N_HPP, reinterpret_cast<Semantic *>(s.data()));
		} else {
			for (index_t i = 0; N != i; ++i) {
	...
	void resize(std::array<size_type, N> new_sizes)
	{
		...
		auto cur_size = std::accumulate(std::cbegin(cur_sizes), std::cend(cur_sizes), N_HPP);
		auto new_size = std::accumulate(std::cbegin(new_sizes), std::cend(new_sizes), N_HPP);
	...
	void resize(index_t const index, size_type new_size)
	{
		...
		auto new_total_size = new_size + size() - size(index) + N_HPP;
	...
	value_t getValue(InputIt first, InputIt last, PropagationCriteria prop_criteria)
	{
		switch (prop_criteria) {
			...
			case PropagationCriteria::MAX: return first->value;
	...
	void read(std::istream &in, IndexField indices, SemanticPropagation const &prop)
	{
		...
		if (N == n) {
			...
			std::array<Semantic, N_HPP> s;
			in.read(reinterpret_cast<char *>(s.data()), N_HPP * sizeof(Semantic));
			for (index_t i = 0, j = 0; N_HPP != i; ++i) {
				sizes[j++] = s[i].label;
				sizes[j++] = s[i].value;
			}
	...
	void write(std::ostream &out) const
	{
		...
		if (empty()) {
			std::array<Semantic, N_HPP> s{};
			out.write(reinterpret_cast<char const *>(s.data()), N_HPP * sizeof(Semantic));
			return;
		}

		out.write(reinterpret_cast<char const *>(data()),
		          (size() + N_HPP) * sizeof(Semantic));
	}
